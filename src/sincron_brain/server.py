...
def _refresh_viewer_if_exists(config: VaultConfig) -> None:
    """Keep _viewer.html in sync after a mutation, but only if the user opted in.

    The viewer is opt-in: it only exists after the user runs `sincron-brain viewer`
    (or it is regenerated by a sleep). When present, agents and users expect it
    to reflect the current vault state, including drafts and reactivation events.
    When absent, this is a no-op so vaults that don't use the viewer pay nothing.
    """
